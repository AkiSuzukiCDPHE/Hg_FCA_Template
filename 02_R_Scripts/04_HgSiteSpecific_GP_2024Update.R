# SITE SPECIFIC ADVISORIES FOR MERCURY FOR GP - 2024 UPDATE
...
# CENSORED STEP Hg_SS <- HgRegression_Final %>% filter (Sample_Year > "2012")
...
Hg_SS2= distinct(Hg_SS2, Waterbody, .keep_all = TRUE) %>%
  select(Waterbody, Species, Old_Species_Codes, Species_Codes, Analyte1, Average_Result, Units,
         Num_Obs, In_Existing_Advisory,Commonly_Consumed,Length_Inches,Pred_Length, FishType)
