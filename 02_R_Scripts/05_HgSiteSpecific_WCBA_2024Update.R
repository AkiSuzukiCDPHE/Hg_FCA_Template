...
# CENSORED Hg_SS <- HgRegression_Final %>% filter (Sample_Year > "2012")
...
Hg_SS2= distinct(Hg_SS2, Waterbody, .keep_all = TRUE) %>%
  select(Waterbody, Species, Old_Species_Codes, Species_Codes, Analyte1, Average_Result, Units, Num_Obs, In_Existing_Advisory,
...
Hg_SS4 <- left_join(Hg_SS3, Hg_Statewide_Final_2024 %>% select(Species_Codes, WCBA_MealsPerMonth),
                    by = "Species_Codes")


# Rename the variables for WCBA_MealsPerMonth
Hg_SS4  <- Hg_SS4  %>%
...
Hg_SS5a_SW <- Hg_SS4 %>% filter(WCBA_MealsPerMonthSW <=8) # Species that have a statewide advisory

Hg_SS5b_NoSW <- Hg_SS4 %>% filter(WCBA_MealsPerMonthSW > 8) # Species that DO NOT have a statewide advisory
...
# (2) Data frame for species without a statewide advisory but with FCLGs <= 8 meals/month
# Hg_SS6_NoSW

Hg_WCBA_SS_Merged <- bind_rows(Hg_SS6_SW, Hg_SS6_NoSW)


# Merge the data frame "Hg_WCBA_SS_Merged" with the existing advisories list
...
# Merge the existing site-specific advisory data frame with with Hg_WCBA_SS_Merged
Hg_WCBA_SS_Merged2 <- left_join(Hg_WCBA_SS_Merged, Hg_ExistingSS %>% select( Waterbody, Species, Population, MealsMonth_ExistingSS, Existing_SS_Advisory),
