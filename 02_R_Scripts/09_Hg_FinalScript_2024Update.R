# FINAL MERCURY SITE-SPECIFIC ADVISORY SCRIPT - USE THIS POST TAC/MANUAL REVIEW
...
HgSS_Final_AllPops <- read_excel("C:/Users/oasuzuki/Documents/R/FCAs/Hg_FCA/04_Output/Hg_2024_Update/Site-specific_Hg_Advisory/SS_TimeNOTCensored_USETHIS/Semi_Final_Dataset/HgSS_Final_AllPops.xlsx")

# Filter out rows with no change i.e. small sample sizes that should not be included in update.
HgSS_Final_AllPops1 <- HgSS_Final_AllPops %>%
  filter((HgSS_Final_AllPops$Decision != "No change") %>% replace_na(TRUE))
...
# Label the decision variable "Lifted" for lifted advisories
HgSS_Final_AllPops_Lifted$Decision <- "Lifted"
...
# Use this export and filter for the waterbodies with new data
# Make changes to the full list of advisories to reflect any updates.
# add any new or updated advisories to the template list for WQ.
# library("writexl")
