...
Hg_SS6a_1 <- Hg_SS6a %>%           
  ...
  mutate(New_SS = case_when(
    Children_MealsPerMonth < MealsMonth_ExistingSS ~ "Yes",
    TRUE ~ "No"
  )) 
...
Hg_SS6b_1 <- left_join(Hg_SS6b, HgData_Statewide1 %>% select(Species_Codes, Children_MealsPerMonth),
                       by = "Species_Codes")


#Renaming the variables for Children_MealsPerMonth
...
Hg_SS6b_1a <- Hg_SS6b_1 %>% filter(Children_MealsPerMonthSW <=8) # Species that have a statewide advisory

Hg_SS6b_1b <- Hg_SS6b_1 %>% filter(Children_MealsPerMonthSW > 8) # Species that DO NOT have a statewide advisory
...
Hg_SS6a_1 <- Hg_SS6a_1 %>% rename(Children_MealsPerMonthSS = Children_MealsPerMonth )
...
Hg_SS_Final <- unique_merged_df %>%
  mutate(Children_SS_MealPerMonthNew = coalesce(Children_MealsPerMonth, Children_MealsPerMonthSS))
